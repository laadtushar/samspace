 ...
 * either, all of which live in Postgres and were working the whole time.
 *
...
/** No database configured, so the read falls through to blob and fails there. */
function mockStoresDown() {
...
describe("GET /api/admin/content while storage is unavailable", () => {
  ...
  it("marks a normal read as stored", async () => {
    ...
    vi.doMock("@/lib/blob", async (importOriginal) => ({
      ...(await importOriginal<typeof import("@/lib/blob")>()),
      readPublicJson: async () => ({ hero: { headline: "Saved copy" } }),
    }));
